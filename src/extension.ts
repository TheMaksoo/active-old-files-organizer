...
class ActiveOldFilesProvider implements vscode.TreeDataProvider<FileItem> {
  ...
  private minFilesForGrouping: number;
  ...
  refresh(): void {
    ...
    this.activeFiles = this.groupByTopLevel(
      allFiles.filter((f) => f.mtime > activeThreshold)
    );
    this.oldFiles = this.groupByTopLevel(
      allFiles.filter((f) => f.mtime <= activeThreshold)
    );
    this.logStats();
  ...
  private logStats() {
    const totalActive = this.activeFiles.reduce(
      (acc, f) => acc + (f.children ? f.children.length : 0),
      0
    );
    const totalOld = this.oldFiles.reduce(
      (acc, f) => acc + (f.children ? f.children.length : 0),
      0
    );
    this.logger.clear();
...
export function activate(context: vscode.ExtensionContext) {
  ...
  if (!workspaceFolders) {
    vscode.window.showWarningMessage(
      "Active/Old Files Organizer: No workspace folder open."
    );
    return;
  }
  const logger = vscode.window.createOutputChannel(OUTPUT_CHANNEL);
  const provider = new ActiveOldFilesProvider(
    ...
    configuration: {
      ...
      properties: {
        ...
        "activeOldOrganizer.minFilesForGrouping": {
          ...
          default: 5,
          description:
            "Minimum files in a folder before creating Active/Old groups",
        },
      },
    },
  };
